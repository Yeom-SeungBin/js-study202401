...
const todos = [
  {
    id: 1,
    text: '할 일 1',
    done: false // checkbox를 클릭해서 할 일을 마쳤는지의 여부(true면 체크 완료)
  }, 
  {
    id: 2,
    text: '할 일 2',
...
function insertTodoData() {
  ...
  // makeNewTodoNode(2번에 생성한 할 일 객체를 전달);
...
function changeCheckState($label) {
  ...
  */
  ...
  /*
    ...
    만약, 기존의 값이 true였다? 그럼 false로 바뀌는 거예요.
  */
...
function removeTodoData($delTarget) {
  ...
  // 배열 내에 있는 개체도 삭제를 진행.
  // 삭제되는 객체가 배열 안에 몇 번째인지를 확인 -> 할 일 완료 처리 함수 쪽에 비슷한 로직이 있습니다.
  // 함수화 시켜보세요.
...
function enterModifyMode($modSpan) {
  ...
  // span.text를 input 태그로 교체 (replaceChild)
  // input 태그에는 .modify-input 를 추가하시고, input에는 기존의 할 일 텍스트가 미리 작성되어 있어야 합니다.
  const $label = $modSpan.parentNode.previousElementSibling;
  const $textSpan = $label.lastElementChild;

  const $modInput = document.createElement('input');
  // $modInput.setAttribute('type', 'text'); (O) input 태그의 type은 text가 기본값이기 때문에 굳이 쓸 필요는 없다. 
...
function modifyTodoData($modCompleteSpan) {
  ...
  // 배열 내의 id가 일치하는 객체를 찾아서 text 프로퍼티 값을 수정된 값으로 변경해 주셔야 합니다.
